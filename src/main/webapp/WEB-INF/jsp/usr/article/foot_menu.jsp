...
<div class="flex flex-1 overflow-hidden">
  ...
  <main class="flex-1 overflow-x-auto p-5">
    <div class="flex gap-5 w-max">
      <c:forEach var="stadium" items="${ftArticles }">
        <div class="w-48 flex-shrink-0 border border-gray-300 rounded-lg overflow-hidden flex flex-col">
          <img src="${stadium.img}" alt="경기장" class="w-full h-32 object-cover">
          <div class="p-2 text-sm">
            <div class="font-semibold">${stadium.stadiumName}</div>
            <div>${stadium.area}</div>
            <div class="text-xs text-gray-500">${stadium.address}</div>
          </div>
        </div>
      </c:forEach>
    </div>
